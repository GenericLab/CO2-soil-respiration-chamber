...
# Renombrar las columnas de CO2 en cada dataframe
nuevo_data1 <- nuevo_data1 %>%
  rename(CO2_1 = CO2)

nuevo_data2 <- nuevo_data2 %>%
  rename(CO2_2 = CO2)

# Crear una lista con los dataframes
dataframes <- list(nuevo_data1, nuevo_data2, nuevo_data3, nuevo_data4, nuevo_data5, nuevo_data6, nuevo_data7, nuevo_data8, nuevo_data9)
...
tiempos_combinados <- sort(unique(c(nuevo_data1$tiempo_transcurrido_numeric, nuevo_data2$tiempo_transcurrido_numeric)))

# Interpolar los valores para CO2_1 y CO2_2 en los tiempos combinados
CO2_1_interpolado <- approx(nuevo_data1$tiempo_transcurrido_numeric, nuevo_data1$CO2_1, xout = tiempos_combinados)$y
CO2_2_interpolado <- approx(nuevo_data2$tiempo_transcurrido_numeric, nuevo_data2$CO2_2, xout = tiempos_combinados)$y

# Crear el dataframe combinado
combined_data <- data.frame(
  tiempo_transcurrido_numeric = tiempos_combinados,
  CO2_1 = CO2_1_interpolado,
  CO2_2 = CO2_2_interpolado
)
...
ggplot(combined_data, aes(x = tiempo_transcurrido_numeric)) +
  ...
  theme_minimal() +
  scale_color_manual(values = c("CO2_1" = "blue", "CO2_2" = "red"))
...
colors <- c(
  ...
  "CO2_4" = "purple", "CO2_5" = "orange", "CO2_6" = "brown", 
  "CO2_7" = "pink", "CO2_8" = "cyan", "CO2_9" = "yellow"
)
...
# Ajustar el modelo de respiración de suelo (Ejemplo usando  )
modelo_respiracion <- tryCatch({
  nlsLM(CO2_1 ~400+ a * (tiempo_transcurrido_numeric / (b + tiempo_transcurrido_numeric)), 
        data = combined_data, 
        start = list(a = max(combined_data$CO2_1), b = median(combined_data$tiempo_transcurrido_numeric)))
}, error = function(e) {
...
if (!is.null(modelo_respiracion)) {
  ...
  combined_data <- combined_data %>%
    mutate(predicted_CO2_1 = predict(modelo_respiracion, newdata = combined_data))
  ...
  ggplot(combined_data, aes(x = tiempo_transcurrido_numeric)) +
    geom_point(aes(y = CO2_1), color = "blue") +
    geom_line(aes(y = predicted_CO2_1), color = "blue", linetype = "dashed") +
    geom_point(aes(y = CO2_2), color = "red") +
    geom_line(aes(y = predicted_CO2_2), color = "red", linetype = "dashed") +
    geom_point(aes(y = CO2_3), color = "black") +
    geom_line(aes(y = predicted_CO2_3), color = "black", linetype = "dashed") +
    geom_point(aes(y = CO2_4), color = "green") +
    geom_line(aes(y = predicted_CO2_4), color = "green", linetype = "dashed") +
    geom_point(aes(y = CO2_5), color = "pink") +
    geom_line(aes(y = predicted_CO2_5), color = "pink", linetype = "dashed") +
    geom_point(aes(y = CO2_6), color = "purple") +
    geom_line(aes(y = predicted_CO2_6), color = "purple", linetype = "dashed") +
    geom_point(aes(y = CO2_7), color = "orange") +
    geom_line(aes(y = predicted_CO2_7), color = "orange", linetype = "dashed") +
    geom_point(aes(y = CO2_8), color = "magenta") +
    geom_line(aes(y = predicted_CO2_8), color = "magenta", linetype = "dashed") +
    geom_point(aes(y = CO2_9), color = "cyan") +
    geom_line(aes(y = predicted_CO2_9), color = "cyan", linetype = "dashed") +
    labs(
      x = "Tiempo Transcurrido (s)",
      y = "CO2",
...
# Ajustar el modelo de respiración de suelo y graficar para cada conjunto de datos CO2_1 a CO2_9
...
# Crear listas para almacenar los datos interpolados y las predicciones
interpolados <- list()
predicciones <- list()

# Ajustar el modelo de Michaelis-Menten y realizar la interpolación para cada conjunto de datos
for (i in 1:9) {
  # Cargar el conjunto de datos
  data <- get(paste0("nuevo_data", i))
  
  # Interpolar los datos
  tiempos_combinados <- sort(unique(data$tiempo_transcurrido_numeric))
  CO2_interpolado <- approx(
    data$tiempo_transcurrido_numeric, 
    data$CO2, 
    xout = tiempos_combinados
  )$y
  
  # Crear dataframe interpolado
  interpolados[[i]] <- data.frame(
    tiempo_transcurrido_numeric = tiempos_combinados,
    CO2 = CO2_interpolado
  )
  
  # Ajustar el modelo de Michaelis-Menten
  modelo <- tryCatch({
    nlsLM(CO2 ~ (Vmax * tiempo_transcurrido_numeric) / (Km + tiempo_transcurrido_numeric), 
          data = interpolados[[i]], 
          start = list(Vmax = 1, Km = 1))
  }, error = function(e) {
    message(paste("Error al ajustar el modelo para CO2_", i, ": ", e, sep = ""))
    NULL
  })
  
  # Realizar predicciones
  if (!is.null(modelo)) {
    predicciones[[i]] <- interpolados[[i]] %>%
      mutate(predicted_CO2 = predict(modelo, newdata = interpolados[[i]]))
  }
}

# Combinar todos los datos interpolados y predicciones en un solo dataframe
combined_data_all <- Reduce(function(x, y) merge(x, y, by = "tiempo_transcurrido_numeric", all = TRUE), interpolados)
predicciones_combined <- Reduce(function(x, y) merge(x, y, by = "tiempo_transcurrido_numeric", all = TRUE), predicciones)

# Añadir las predicciones al dataframe combinado
combined_data_all <- left_join(combined_data_all, predicciones_combined, by = "tiempo_transcurrido_numeric", suffix = c("_real", "_pred"))
...
combined_data <- combined_data %>%
  mutate(CO2_combined = (CO2_1 + CO2_2 + CO2_3 + CO2_4 + CO2_5+ CO2_6+ CO2_7 + CO2_8 +CO2_9) / 9)
