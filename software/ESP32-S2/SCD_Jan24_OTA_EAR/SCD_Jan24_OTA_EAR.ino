...
float light;
UMS3 ums3;
const int LEDonBoard = 1;       // the number of the LED pin
//const int LEDexternal = 33;      // the number of the LED pin
const int LEDexternal = 38;      // 2nd LDO on GPIO21
const int buttonPin = 18;        // the number of the pushbutton pin
const int cal_pin = 0;           // entrada pulsador calibración
//const int neoPixelPinOnBoard = 40;          // NeoPixel for UROS board
const int neoPixelPin = 7;          // NeoPixel for UROS board
#define SDA_PIN 9
#define SCL_PIN 8
#endif
...
//uint16_t alt = 1170; //Girlen, Switzerland
//uint16_t alt = 310; //Chiangmai, Thailand
//uint16_t alt = 15; //Ratchaburi, Thailand
...
void loop() {
    ...
    if ((millis() - lastDebounceTime) > debounceDelay) {
      if (reading != buttonState) {
        ...
        if (screenState == 0) {
          displayLimit = 1500;
          displayLowerLimit = 0;
          displayFactor = (displayLimit-displayLowerLimit) / 48;
          OLEDdrawBackground();
    ...
    #ifdef FEATHERS3
    ums3battery = ums3.getBatteryVoltage();
    voltage = voltage + (ums3battery*1000); 
    #endif

    #ifdef WEMOS_LOLIN32
    voltage = voltage + map(ADCValue, 0, 1023, 0, 2580); 
    #endif
    ...
    if (millis() - DS18x20DataTimer >= 1000){
      ...
      //light = ums3.getLightSensorVoltage();
      printResults();
    }    
#endif
  ...
  lastButtonState = reading;
      ...
      if (millis() - thingspeakDataTimer >= 30000){
        ...
        Serial.println("Sending measurements");
          ...
          ThingSpeak.setField(6, battery/1000);
         #endif
         #ifdef DS18x20_IS_ATTACHED
    ...
    if (screenState == 1) {
        ...
        OLEDgraphTEMP(temperature, co2, humidity);
      }
      //printResults();
    }
}
...
*/
    if (meas_counter >= 0) {
        display.drawLine(meas_counter + 28, 48, meas_counter + 28, 48 - ((temp*100)-displayLowerLimit) / displayFactor, WHITE);
    }
